...
$now = date('Y-m-d');
$Sql = "SELECT cuti.*, employee.* FROM cuti, employee WHERE cuti.username=employee.username AND cuti.no_cuti='$_GET[no]'";
					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<input type="text" name="mulai" class="form-control" value="<?php echo $data['izin']; ?> " readonly>
								</div>
							</div>
							<?php if (isset($data) && $data['izin'] == 'cuti' || $data['izin'] == 'cuti khusus' || $data['izin'] == 'libur pengganti') { ?>
								<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<input type="text" name="mulai" class="form-control" value="<?php echo $data['lokasi_kerja']; ?> " readonly>
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<label class="control-label col-sm-3">HOD</label>
								<div class="col-sm-4">
									<select name="mng" id="mng" class="form-control" disabled>
										<?php
										$sql_don = "SELECT * FROM employee WHERE hak_akses='Hod' AND active='Aktif' ORDER BY nama_emp ASC";
										$ress_don = mysqli_query($conn, $sql_don);
										while ($li = mysqli_fetch_array($ress_don)) {
											echo '<option value="' . $li['username'] . '">' . $li['nama_emp'] . '</option>';
										}
										?>
									</select>
								</div>
