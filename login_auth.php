	...
	if(empty($_POST['username']) || empty($_POST['password'])) {
		...
		$password = htmlentities(trim(strip_tags($password)));
			...
			if($akses=="Admin"){			
				...
				$aks = "Leader";
				$sql = "SELECT * FROM employee WHERE hak_akses='".$aks."' AND username='". $username ."' AND password='". $password ."'";
				$ress = mysqli_query($conn, $sql);
				$rows = mysqli_num_rows($ress);
				$dataku = mysqli_fetch_array($ress);
				// mendaftarkan session jika username di temukan
				if($rows == 1) {
					// membuat variabel session
					$_SESSION['leader'] = strtolower($dataku['username']);
					// mengarahkan ke halaman indeks.php
				...
				if($rows == 1) {
					// membuat variabel session
					$_SESSION['manager'] = strtolower($dataku['username']);
					// mengarahkan ke halaman indeks.php
				...
				if($rows == 1) {
					// membuat variabel session
					$_SESSION['pegawai'] = strtolower($dataku['username']);
					// mengarahkan ke halaman indeks.php
					header("location: pegawai/index.php?login=success");
				}else{
					header("location: login.php?err=not_found");
				}
			}else if($akses=="non_staff_borongan"){		
				$aks = "non_staff_borongan";
				$sql = "SELECT * FROM employee WHERE hak_akses='".$aks."' AND username='". $username ."' AND password='". $password ."'";
				$ress = mysqli_query($conn, $sql);
				$rows = mysqli_num_rows($ress);
				$dataku = mysqli_fetch_array($ress);
				...
				if($rows == 1) {
					// membuat variabel session
					$_SESSION['non_staff_borongan'] = strtolower($dataku['username']);
					// mengarahkan ke halaman indeks.php
					header("location: non_staff_borongan/index.php?login=success");
				}else{
					header("location: login.php?err=not_found");
				}
			}else if($akses=="non_staff_bulanan"){		
				$aks = "non_staff_bulanan";
				$sql = "SELECT * FROM employee WHERE hak_akses='".$aks."' AND username='". $username ."' AND password='". $password ."'";
				$ress = mysqli_query($conn, $sql);
				$rows = mysqli_num_rows($ress);
				$dataku = mysqli_fetch_array($ress);
				...
				if($rows == 1) {
					// membuat variabel session
					$_SESSION['non_staff_bulanan'] = strtolower($dataku['username']);
					// mengarahkan ke halaman indeks.php
					header("location: non_staff_bulanan/index.php?login=success");
				}else{
					header("location: login.php?err=not_found");
				}
			}else{			
				...
				if($rows == 1) {
					// membuat variabel session
					$_SESSION['supervisor'] = strtolower($dataku['username']);
					// mengarahkan ke halaman indeks.php
