				...
				<div class="panel panel-default">
					<div class="panel-body">
						...
						if ($sess_jabatan == 'Staff HRD') {
							$Sql = "SELECT cuti.*, employee.* FROM cuti, employee WHERE cuti.username=employee.username AND 
									cuti.stt_cuti='Menunggu Approval HRD' ORDER BY cuti.tgl_pengajuan DESC";
							$Qry = mysqli_query($conn, $Sql);
						} else {
							$Sql = "SELECT cuti.*, employee.* FROM cuti, employee WHERE cuti.username=employee.username AND cuti.direksi = '$sess_admuser' AND
										cuti.stt_cuti='Menunggu Approval Direksi' ORDER BY cuti.tgl_pengajuan DESC";
							$Qry = mysqli_query($conn, $Sql);
						}
						?>
						<table class="table table-striped table-bordered table-hover" id="tabel-data">
							...
							<tbody>
								...
								while ($data = mysqli_fetch_array($Qry)) {
									...
									echo '<td class="text-center"><a href="#myModal" data-toggle="modal" data-load-username="' . $data['username'] . '" data-remote-target="#myModal .modal-body">' . $data['nama_emp'] . '</a></td>';
...
<script>
	...
	$('[data-load-username]').on('click', function(e) {
		e.preventDefault();
		var $this = $(this);
		var username = $this.data('load-username');
		if (username) {
			$($this.data('remote-target')).load('karyawan_detail.php?code=' + username);
