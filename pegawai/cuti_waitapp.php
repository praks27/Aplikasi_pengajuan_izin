				...
				<div class="panel panel-default">
					<div class="panel-body">
						...
						$Sql = "SELECT cuti.*, employee.* FROM cuti, employee WHERE cuti.username=employee.username AND cuti.stt_cuti !='Rejected'
									    AND cuti.username='$id' ORDER BY cuti.wkt_pengajuan DESC";
						...
						<table class="table table-striped table-bordered table-hover" id="tabel-data">
							...
							<tbody>
								...
								while ($data = mysqli_fetch_array($Qry)) {
									...
									<a href="cuti_delete.php?no=<?php echo $data['no_cuti']; ?>&username=<?= $data['username']; ?>" onclick="return confirm('Apakah anda yakin akan membatalkan pengajuan izin No. <?php echo $data['no_cuti']; ?>?');" class="btn btn-danger btn-xs btn_hps">Hapus</a></td>
								<?php
									echo '</td>';
									echo '</tr>';
									$i++;
